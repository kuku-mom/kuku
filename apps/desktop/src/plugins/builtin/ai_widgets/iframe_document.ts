...
function buildWidgetIframeDocument(project: WidgetProject): string {
  ...
  if (/<html[\s>]/i.test(body)) {
    if (/<head(\s[^>]*)?>/i.test(body)) {
      return body.replace(/<head(\s[^>]*)?>/i, (match) => `${match}${cspMeta()}`);
    }
    return body.replace(/<html(\s[^>]*)?>/i, (match) => `${match}<head>${cspMeta()}</head>`);
  }
...
function completeHtmlDocument(body: string): string {
  return `<!doctype html><html><head>${cspMeta()}<style>html,body{margin:0;min-height:100%;background:transparent;color:CanvasText;font:13px system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}main{display:block}</style></head><body>${body}</body></html>`;
}
