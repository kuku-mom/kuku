...
const WIDGET_AUTO_RESIZE =
  '(()=>{let last=0;const measure=()=>{const body=document.body;const html=document.documentElement;return Math.ceil(Math.max(body?.scrollHeight??0,body?.offsetHeight??0,html?.scrollHeight??0,html?.offsetHeight??0))};const post=()=>{const height=measure();if(height>0&&height!==last){last=height;parent.postMessage({type:"kuku-widget:resize",height},"*")}};const start=()=>{post();requestAnimationFrame(post);try{const observer=new ResizeObserver(post);observer.observe(document.documentElement);if(document.body)observer.observe(document.body)}catch{setInterval(post,500)}};if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",start,{once:true})}else{start()}addEventListener("load",post)})();';
