...
pub fn build_system_prompt(mode: ChatMode, tools: &[ToolDescriptor]) -> String {
    ...
    let widget_instruction = if tools.iter().any(|tool| tool.name == "create_widget") {
        "\n\nWidget embeds: Call list_widgets before create_widget for requests that may match an existing widget. If a suitable widget exists, reuse its markdownEmbed and do not call create_widget unless the user asks to change it. User requests override these widget defaults; follow the user's explicit request if they ask for borders, shadows, fixed sizing, accent colors, internal scrolling, or another visual style. Make new widgets interactive whenever possible. JavaScript is allowed for local DOM interaction, timers, calculations, and widget UI state. Do not use inline event handlers, external URLs, network APIs, navigation APIs, iframes, forms, top navigation, dynamic code generation, or computed access to browser globals; use inline data/blob assets only. For widget visual design, unless the user explicitly asks for a visual style, use a minimal black, white, and gray design system. By default, unless the user explicitly asks otherwise, avoid border, outline, box-shadow, and decorative ::before on the root or main widget box. Do not introduce accent colors unless the user's prompt requests them. When create_widget returns markdownEmbed, insert that exact fenced kuku-widget block into the note. Use edit_file for the note edit. Do not use provider-side file patching for note edits. Never insert raw iframe HTML for widgets."
    } else {
        ""
    };
...
mod tests {
    ...
    fn widget_prompt_defaults_to_minimal_grayscale_design() {
        ...
        assert!(prompt.contains("border"));
        assert!(prompt.contains("outline"));
        assert!(prompt.contains("box-shadow"));
        assert!(prompt.contains("::before"));
        assert!(prompt.contains("unless the user explicitly asks otherwise"));
    }
}
