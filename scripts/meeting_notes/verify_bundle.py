...
def verify(bundle: Path) -> None:
    ...
    for name in (
        ...
        "requirements.lock", "model_manifest.json", "runtime_manifest.txt",
        "THIRD_PARTY_NOTICES.md",
    ):
        assert (resources / "resources/meeting_notes/asr" / name).is_file(), name
    asr_resources = resources / "resources/meeting_notes/asr"
    runtime_manifest = dict(
        line.split("=", 1)
        for line in (asr_resources / "runtime_manifest.txt").read_text("utf-8").splitlines()
    )
    assert runtime_manifest == {
        "python": "3.12.13",
        "requirements_sha256": hashlib.sha256(
            (asr_resources / "requirements.lock").read_bytes()
        ).hexdigest(),
    }, "Runtime manifest must match the bundled dependency lock"
    model_manifest = json.loads(
        (asr_resources / "model_manifest.json").read_text("utf-8")
    )
    assert set(model_manifest) == {"asr", "diarization"}, "Model manifest is incomplete"
    assert all(
        model.get("revision") and model.get("files")
        for model in model_manifest.values()
    )
